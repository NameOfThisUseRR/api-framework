...
export default class LikeApi extends Client {
   ...
   */
  static async likes(id: number, voice: { like: boolean}): Promise<AxiosResponse<Cat>> {
  ...
  static async dislikes(id: number, voice: { dislike: boolean }): Promise<AxiosResponse<Cat>> {
    let response: AxiosResponse;
    try {
      response = await this.coreApiHttpClient.post(`${this.api}/${id}/likes`, voice);
    } catch (error) {
